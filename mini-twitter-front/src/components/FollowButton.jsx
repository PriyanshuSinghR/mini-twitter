        ...
        <div className="ml-auto">
          {followers.includes(`${currentUser.id}`) ? (
            <button
              ...
              onClick={follow}
              className="w-[120px] h-[30px] rounded-3xl border border-gray-600 flex items-center justify-center transition-all ml-auto hover:bg-red-600 hover:bg-opacity-10 "
            >
              {fLoading ? <div className="w-4 h-4 border-2 border-gray-600 border-t-[#1d9bf0] animate-spin rounded-full "></div> : <span className="text-white text-sm font-semibold "> {text} </span>}
            </button>
          ) : (
